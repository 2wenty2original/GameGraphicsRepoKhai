	...
	for (float& exposure : exposureValues) {
		...
		for (int r = 0; r < height; ++r) 
			for (int c = 0; c < width; ++c) {
				...
				int blueComp = static_cast<int>(Compress(static_cast<float>(blue) * exposure / 500) * 255);
				int greenComp = static_cast<int>(Compress(static_cast<float>(green) * exposure / 500) * 255);
				int redComp = static_cast<int>(Compress(static_cast<float>(red) * exposure / 500) * 255);



				TGAColor color(blueComp, greenComp, redComp, 255);
				image.set(c, r, color);
